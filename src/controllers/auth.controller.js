...
const login = async (req, res) => {
  ...
  try {
    ...
    const registrosUsuarios = await usuariosModel.m_base_SELECT('usuarios', '*', { 'nombre_usuario': nombre_usuario });
    ...
    const token = jwt.sign(payload, env.SECRET_KEY, { expiresIn: '1h' });
...
const refresh = (req, res) => {
  ...
  res.send('Obtener un nuevo token');
};
